...
function travBasin([int]$row,[int]$col,[int]$parentPos){
   $i = ($row*$cols)+$col
   $current = $matrix[$i]   
   ...
   if($row -gt 0)
   {
      $pos = $i -$cols
      $top = $matrix[ $pos]
      if($top -ge $current -and $pos -ne $parentPos)
      {  
       
         $travList =  travBasin $($row-1) $col $i
   ...
   if($row -lt $rows -1)
   {
      $pos =  $i +$cols
      $down = $matrix[$pos]
      if($down -ge $current  -and $pos -ne $parentPos)
      {  
   ...
   if($col -gt 0)
   {
      $pos =  $i -1
      $left = $matrix[ $pos]
      if($left -ge $current -and $pos -ne $parentPos )
      {  
           
           $travList = travBasin $row $($col-1) $i
         foreach($trav in $travList)
         { 
           if(!$size.Contains($trav))
           {
   ...
   if($col -lt $cols-1){
     $pos =  $i +1
     $right = $matrix[ $pos]
      if($right -ge $current -and $pos -ne $parentPos)
      {  
